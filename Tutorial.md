...
This creates a new `MutationSummary` that begins observing the entire document immediately. `MutationSummary` requires only two things to work: the function to call when it has changes to report, and [at least one “query”](APIReference#Configuration_Options.md) of what it should be watching for. In this case, we’re only asking for a single [attribute query](APIReference#The_attribute_Query.md) and giving it the name of the attribute we’re looking for.
...
But our library also wants to respond to new widgets that appear in the page, possibly created by script or a templating library. In order to accomplish this, we’d create an [element query](APIReference#The_element_Query.md) like this:
...
The [element query](APIReference#The_element_Query.md) instructs the `MutationSummary` to watch for particular elements. The contents of the `element` string can be a [very simple subset of CSS](APIReference#Supported_Selector_Syntax.md). This allows you to watch literal elements or do something more complex, as in this example.
...
# Learning More #
  ...
  * Refer to the [API Reference](APIReference.md) document -- in particular to learn about the two other types of queries: [characterData](APIReference#The_characterData_Query.md) and [all](APIReference#The_all_Query.md).
  * Examine the [PageMirror](https://code.google.com/p/mutation-summary/source/browse/#git%2Fexamples%2Fpagemirror_extension) extension which shows how to use the all query and the included TreeMirror utility class to fully mirror the contents of a document.