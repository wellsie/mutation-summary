...
It's built on top of (and requires) a new browser API called [DOM Mutation Observers](http://dom.spec.whatwg.org/#mutation-observers).
  * [Browsers which currently implement DOM Mutation Observers](DOMMutationObservers#Browser_Availability.md).
...
The intent here isn't to be all things to all use-cases. Mutation Summary is not meant to:
  ...
  * **Observing complex selectors.** It offers support for a simple [subset of CSS selectors](APIReference#Supported_Selector_Syntax.md). Want to observe all elements that match `“div[foo] span.bar > p:first-child”`? Unfortunately, efficiently computing that is much harder and currently outside the scope of this library.
...
# Make Contact #
  ...
  * [File a bug](http://code.google.com/p/mutation-summary/issues/entry)
  * [Join the discussion](https://groups.google.com/group/mutation-summary-discuss)